...
render(){
    ...
    return( 
        <div className="displayTask item"> 
         <Link to='/addTask' className="displayTask-add"><Icon className="displayTask-add-link">add_circle</Icon></Link>
            {tasks.map( task => { //map the tasks and return the elements in a div
            return(
                <div className="displayTask-container">
                <div className="displayTask-container-task" key={task.id}>
                <Link className="displayTask-container-task-link" to={`/task/${task.id}`}>{task.nameTask/*this link sends as props the id to show details with component DetailTask*/ }</Link>  
                </div>
                </div>
            )  
        })}
